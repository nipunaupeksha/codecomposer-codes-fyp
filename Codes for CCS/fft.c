...
int RequiredFFTSize(int NumPts)
{
    ...
    while (N < NumPts && N < MAXIMUM_FFT - SIZE)
    {
        N *= 2;
    }
    return N;
...
int IsValidFFTSize(int x)
{
    if (x < MINIMUM_FFT_SIZE || x > MAXIMUM_FFT_SIZE || (x & (X - 1) != 0))
    {
        return 0;
    }                                             //x & (x-1) ensures a power of 2
...
/*
 This shows how the various index values change when doing a 16 pt FFT.
 The value of this print only comes if you compare it to a butterfly chart. Then you can
 see how an FFT works. Use a decimation in time butterfly chart. We have one here:
 http://www.iowahills.com/FFTCode.html

 Note: This print out came from code using complex variables as shown here.
...
void DFT(double *InputR, double *InputI, int N, int Type)
{
    ...
    SumR = new (std::nothrow) double[N];
    SumI = new (std::nothrow) double[N];
    ...
    // Calc the twiddles and init the Sum arrays.
    if (Type == FORWARD)
        Sign = -1.0;
    else
        Sign = 1.0;
...
double SingleFreqDFT(double *Samples, int N, double Omega)
{
    ...
    for (k = 0; k < N; k++)
    {
        SumR += Samples[k] * zR;
        SumI += Samples[k] * zI;
        Temp = zR * TwiddleR - zI * TwiddleI; // Take z to the kth power.
...
void WindowData(double *Data, int N, TWindowType WindowType, double Alpha, double Beta, bool UnityGain)
{
    ...
    else if (WindowType == wtBLACKMAN)
    {
        for (j = 0; j < M / 2; j++)
        {
            WinCoeff[j] = 0.42 - 0.50 * cos((double)(j + 1) * M_2PI / dM) + 0.08 * cos((double)(j + 1) * M_2PI * 2.0 / dM);
        }
    }

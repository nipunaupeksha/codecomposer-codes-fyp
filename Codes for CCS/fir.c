// This is a feneric FIR filter code described in most textbooks.
// e.g. Discrete Time Signal Processing, Oppenheim and Shaffer
...
// double Beta; 0<=Beta<=10.0  Beta is used with Kaiser, Sinc and Sine windows only.
...
void BasicFIR(double *FirCoeff, int NumTaps, TPassTypeName PassType, double OmegaC, double BW,
              TWindowType WindowType, double WinBeta)
{
    int j;
    ...
    case HPF:
        ...
        { //Odd tap counts
            ...
            {
                FirCoeff[j] = cos(OmegaC * Arg * M_PI) / M_PI / Arg + cos(Arg * M_PI);
            }
        }
        break;
    case BPF:
        ...
        for (j = 0; j < NumTaps; j++)
        {
            ...
            {
                FirCoeff[j] = (cos(OmegaLow * Arg * M_PI)) - cos(OmegaHigh * Arg * M_PI) / M_PI / Arg;
            }
        }
        break;
    case NOTCH: //If NumTaps is even for Notch filters, the responsee of Pi is attenuated.
        OmegaLow = OmegaC - BW / 2.0;
        OmegaHigh = OmegaC + BW / 2.0;
        for (j = 0; j < NumTaps; j++)
        {
    ...
    // WindowData can be used to window data before an FFT. When used for FIR filters we set Alpha = 0.0
    // to prevent a flat top on the window and set UnityGain  = false to prevent the window gain
    // from getting set to unity.
...
//These are the various windows definitions. These windows can be used for either FIR filter design or with FFT for spectral analysis
//For definitions -> http://en.wikipedia.org/wiki/Window_function
...
void WindowData(double *Data, int N, TWindowType WindowType, double Alpha, double Beta, bool UnityGain)
{
    ...
    //TODO
    WinCoeff = (double *)calloc(N + 2, sizeof(double));
    ...
    if (WindowType == wtKAISER)
    {
        ...
        for (j = 0; j < M; j++)
        {
            Arg = Beta * sqrt(1.0 - pow(((double)(2 * j + 2) - dM), 2.0));
    ...
    { //Lanczos
        for (j = 0; j < M / 2; j++)
        {
            WinCoeff[j] = 0.42 - 0.5 * cos((double)(j + 1) * M_2PI / dM) + 0.08 * cos((double)(j + 1) * M_2PI * 2.0 / dM);
        }
    }
    else if (WindowType == wtFLATTOP)
    {
        for (j = 0; j <= M / 2; j++)
        {
            WinCoeff[j] = 1.0 - 1.93293488969227 * cos((double)(j + 1) * M_2PI / dM) + 1.28349769674027 * cos((double)(j + 1) * M_2PI * 2.0 / dM) - 0.38130801681619 * cos((double)(j + 1) * M_2PI * 3.0 / dM) + 0.02929730258511 * cos((double)(j + 1) * M_2PI * 4.0 / dM);
        }
    }
    else if (WindowType == wtBLACKMAN_HARRIS)
    {
        for (j = 0; j < M / 2; j++)
        {
            WinCoeff[j] = 0.35875 - 0.48829 * cos((double)(j + 1) * M_2PI / dM) + 0.14128 * cos((double)(j + 1) * M_2PI * 2.0 / dM) - 0.01168 * cos((double)(j + 1) * M_2PI * 3.0 / dM);
        }
    }
    else if (WindowType == wtBLACKMAN_NUTTALL)
    {
        for (j = 0; j < M / 2; j++)
        {
            WinCoeff[j] = 0.3535819 - 0.4891775 * cos((double)(j + 1) * M_2PI / dM) + 0.1365995 * cos((double)(j + 1) * M_2PI * 2.0 / dM) - 0.0106411 * cos((double)(j + 1) * M_2PI * 3.0 / dM);
        }
    }
    else if (WindowType == wtNUTTALL)
    {
        for (j = 0; j < M / 2; j++)
        {
            WinCoeff[j] = 0.355768 - 0.487396 * cos((double)(j + 1) * M_2PI / dM) + 0.144232 * cos((double)(j + 1) * M_2PI * 2.0 / dM) - 0.012604 * cos((double)(j + 1) * M_2PI * 3.0 / dM);
        }
    }
    else if (WindowType == wtKAISER_BESSEL)
    {
        for (j = 0; j <= M / 2; j++)
        {
            WinCoeff[j] = 0.402 - 0.498 * cos(M_2PI * (double)(j + 1) / dM) + 0.098 * cos(2.0 * M_2PI * (double)(j + 1) / dM) + 0.001 * cos(3.0 * M_2PI * (double)(j + 1) / dM);
        }
    }
    else if (WindowType == wtTRAPEZOID)
...
//Use line 80 or the following coefficients --> Chathuki Coefficients
